...
describe('Acronym e2e test', () => {
  ...
  const acronymSample = { termOrAcronym: 'durian drat straight' };
  ...
  describe('new Acronym page', () => {
    ...
    it('should create an instance of Acronym', () => {
      cy.get(`[data-cy="termOrAcronym"]`).type('almanac giddy considering');
      cy.get(`[data-cy="termOrAcronym"]`).should('have.value', 'almanac giddy considering');

      cy.get(`[data-cy="name"]`).type('meanwhile');
      cy.get(`[data-cy="name"]`).should('have.value', 'meanwhile');

      cy.get(`[data-cy="definition"]`).type('tidy instead');
      cy.get(`[data-cy="definition"]`).should('have.value', 'tidy instead');
